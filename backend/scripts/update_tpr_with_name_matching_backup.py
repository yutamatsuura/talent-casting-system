...
# TPRファイルと対応するターゲット層ID
TPR_FILES_MAPPING = {
    "TPR_男性12～19_202508.csv": 13,
    "TPR_女性12～19_202508.csv": 9,
    "TPR_男性20～34_202508.csv": 14,
    "TPR_女性20～34_202508.csv": 10,
    "TPR_男性35～49_202508.csv": 15,
    "TPR_女性35～49_202508.csv": 11,
    "TPR_男性50～69_202508.csv": 16,
    "TPR_女性50～69_202508.csv": 12,
}
        ...
        async with get_session_maker()() as session:
            try:
                for record in records:
                    ...
                    result = await session.execute(
                        select(TalentScore)
                        .where(TalentScore.account_id == record['account_id'])
                        .where(TalentScore.target_segment_id == record['target_segment_id'])
                    )
                    talent_score = result.scalar_one_or_none()

                    if talent_score:
                        # 既存レコード更新
                        talent_score.tpr_power_score = Decimal(str(record['tpr_power_score']))

                        # base_power_score自動計算（COALESCE方式）
                        vr_val = talent_score.vr_popularity if talent_score.vr_popularity else Decimal('0')
                        tpr_val = Decimal(str(record['tpr_power_score']))
                        talent_score.base_power_score = (vr_val + tpr_val) / 2

                    else:
                        # 新規レコード作成（VRデータがない場合）
                        tpr_val = Decimal(str(record['tpr_power_score']))
                        base_val = (Decimal('0') + tpr_val) / 2  # tpr_power_score / 2

                        new_score = TalentScore(
                            account_id=record['account_id'],
                            target_segment_id=record['target_segment_id'],
                            vr_popularity=None,
                            tpr_power_score=tpr_val,
                            base_power_score=base_val
                        )
