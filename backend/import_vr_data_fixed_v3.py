...
async def upsert_talent_score(session, talent_id, target_segment_id, vr_popularity, tpr_power_score=None):
    """TalentScoreのUPSERT処理"""
    # PostgreSQLのON CONFLICT構文を使用
    stmt = text("""
        INSERT INTO talent_scores (talent_id, target_segment_id, vr_popularity, tpr_power_score, base_power_score, created_at)
        VALUES (:talent_id, :target_segment_id, :vr_popularity, :tpr_power_score,
                CASE
                    WHEN :vr_popularity IS NOT NULL AND :tpr_power_score IS NOT NULL
                    THEN (:vr_popularity + :tpr_power_score) / 2
                    ELSE NULL
                END,
                NOW())
        ...
        DO UPDATE SET
            ...
            base_power_score = CASE
                WHEN EXCLUDED.vr_popularity IS NOT NULL AND talent_scores.tpr_power_score IS NOT NULL
                THEN (EXCLUDED.vr_popularity + talent_scores.tpr_power_score) / 2
                ELSE talent_scores.base_power_score
            END
    """)
    ...
    await session.execute(stmt, {
        ...
        "vr_popularity": vr_popularity,
        "tpr_power_score": tpr_power_score
    })

async def upsert_talent_image(session, talent_id, target_segment_id, image_item_id, score):
    """TalentImageのUPSERT処理"""
    stmt = text("""
        INSERT INTO talent_images (talent_id, target_segment_id, image_item_id, score, created_at)
        VALUES (:talent_id, :target_segment_id, :image_item_id, :score, NOW())
        ON CONFLICT (talent_id, target_segment_id, image_item_id)
...
async def import_vr_data():
    """VRデータインポート（修正版v3 - 重複対応）"""
    print("\n📥 Importing VR data (Fixed version v3 - Duplicate Safe)...")
            ...
            async with await get_async_session() as session:
                for _, row in df.iterrows():
                    ...
                    if pd.notna(popularity):
                        # TalentScore UPSERT（VR人気度）
                        await upsert_talent_score(session, talent_id, target_segment_id, Decimal(str(popularity)))
                        total_vr_scores += 1
                    ...
                    for image_name, col_index in image_columns:
                        if image_name in image_mapping and col_index < len(row):
                            ...
                            if pd.notna(image_score):
                                await upsert_talent_image(
                                    session, talent_id, target_segment_id,
                                    image_mapping[image_name], Decimal(str(image_score))
                                )
...
async def main():
    """メイン処理"""
    print("=" * 60)
    print("🚀 Starting VR data import (Fixed version v3 - Duplicate Safe)...")
    print("=" * 60)
