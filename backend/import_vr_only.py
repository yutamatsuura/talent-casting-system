#!/usr/bin/env python3
"""VRデータのみインポートスクリプト（最終版 - 既存TPR手法活用）"""
...
async def import_vr_data():
    """VRデータインポート（既存TPR手法活用）"""
    print("\n📥 Importing VR data (Final version using existing TPR approach)...")
        ...
        for csv_file in csv_files:
            ...
            async with await get_async_session() as session:
                for _, row in df.iterrows():
                    # タレント名（2番目の列）
                    talent_name = str(row.iloc[1]).strip() if len(row) >= 2 else None
                    ...
                    # 人気度（3番目の列）
                    popularity = pd.to_numeric(row.iloc[2], errors="coerce") if len(row) >= 3 else None

                    if pd.notna(popularity):
                        # 既存TalentScoreの更新（TPR手法を活用）
                        ...
                        if talent_score:
                            ...
                            if talent_score.tpr_power_score:
                                talent_score.base_power_score = (
                                    talent_score.vr_popularity + talent_score.tpr_power_score
                                ) / 2
                        else:
                    ...
                    # イメージデータ処理
                    image_columns = [
                        ("おもしろい", 4), ("清潔感がある", 5), ("個性的な", 6),
                        ("信頼できる", 7), ("かわいい", 8), ("カッコいい", 9), ("大人の魅力がある", 10)
                    ]
                    ...
                    for image_name, col_index in image_columns:
                        if image_name in image_mapping and col_index < len(row):
                            ...
                            if pd.notna(image_score):
                                ...
                                if existing_record:
                                    # 既存レコードを更新
                                    existing_record.score = Decimal(str(image_score))
                                else:
                                    # 新しいレコードを追加
...
async def main():
    """メイン処理"""
    print("=" * 60)
    print("🚀 Starting VR data import (Final version using existing TPR approach)...")
    print("=" * 60)
