...
async def analyze_step0_budget_filtering():
    print("🔍 STEP 0: 予算フィルタリング詳細分析（修正版）")
    print("=" * 70)
    ...
    try:
        ...
        table_check = await conn.fetch("""
            ...
            WHERE table_name IN ('m_account', 'm_talent_act')
        ...
        UNION ALL
        SELECT
            'talent_scores' as table_name,
            COUNT(*) as record_count,
            COUNT(DISTINCT account_id) as unique_ids,
            'スコア情報' as description
        FROM talent_scores
        """
        ...
        # 7. 実際のマッチングロジック使用確認
        print("\n\n7️⃣ matching.pyの実際の実装確認:")

        actual_matching_query = """
        SELECT
            ma.account_id,
            ma.name_full_for_matching,
            mta.money_max_one_year,
            -- 実際のmatching.pyのロジック
            CASE
                WHEN 2999 = 'Infinity'::float8 OR mta.money_max_one_year IS NULL THEN 'PASS (NULL)'
                WHEN mta.money_max_one_year <= 2999 THEN 'PASS (範囲内)'
                ELSE 'REJECT (範囲外)'
            END as actual_filter_result
        FROM m_account ma
        LEFT JOIN m_talent_act mta ON ma.account_id = mta.account_id
        WHERE ma.del_flag = 0
        ORDER BY mta.money_max_one_year DESC NULLS LAST
        ...
        actual_results = await conn.fetch(actual_matching_query)
        print("\n   1,000万円〜3,000万円未満での実際のマッチング結果:")
        print("   ID   | Name         | 年間契約上限 | matching.py判定")
        print("   " + "-" * 65)

        for result in actual_results:
            name = (result['name_full_for_matching'] or 'Unknown')[:12].ljust(12)
            amount = result['money_max_one_year'] or 'NULL'
            status = result['actual_filter_result']
            print(f"   {result['account_id']:>4} | {name} | {str(amount):>10} | {status}")
